...
describe("token integration", async () => {
  ...
  const [tokenAdmin, newTokenAdmin, tokenA, tokenBAdmin, tokenB, thirdPartyA, thirdPartyB] =
    ...
    it("should deploy token contract B", async () => {
      const tx = await Mina.transaction(
        ...
        async () => {
          ...
          await tokenBContract.initialize(tokenBAdmin, UInt8.from(9), Bool(false));
        }
      );
  ...
  describe("admin", () => {
    ...
    it("should mint for the sender and receiver account", async () => {
      const initialBalance = (await tokenAContract.getBalanceOf(sender)).toBigInt();
  ...
  describe("transfers", () => {
    ...
    it("should do a transfer initiated by the token contract", async () => {
      const initialBalanceSender = (await tokenAContract.getBalanceOf(sender)).toBigInt();
      const initialBalanceReceiver = (await tokenAContract.getBalanceOf(receiver)).toBigInt();
      const initialCirculating = (await tokenAContract.getCirculating()).toBigInt();
      ...
      equal((await tokenAContract.getCirculating()).toBigInt(), initialCirculating);
    });
    ...
    it("should do a transaction constructed manually, approved by the token contract", async () => {
      ...
      const updateSend = AccountUpdate.createSigned(sender, tokenAContract.deriveTokenId());
      updateSend.balanceChange = Int64.fromUnsigned(sendAmount).neg();
      const updateReceive = AccountUpdate.create(receiver, tokenAContract.deriveTokenId());
    ...
    it("rejects transactions with mismatched tokens", async () => {
      const updateSend = AccountUpdate.createSigned(sender, tokenAContract.deriveTokenId());
      updateSend.balanceChange = Int64.fromUnsigned(sendAmount).neg();
      const updateReceive = AccountUpdate.create(receiver, tokenBContract.deriveTokenId());
      updateReceive.balanceChange = Int64.fromUnsigned(sendAmount);
      await rejects(
        async () => (
          await Mina.transaction(
    ...
    it("Should reject manually constructed transfers from the account that's tracking circulation", async () => {
      const updateSend = AccountUpdate.createSigned(tokenA, tokenAContract.deriveTokenId());
      updateSend.balanceChange = Int64.fromUnsigned(sendAmount).neg();
      const updateReceive = AccountUpdate.create(receiver, tokenAContract.deriveTokenId());
      updateReceive.balanceChange = Int64.fromUnsigned(sendAmount);
    ...
    it("Should reject manually constructed transfers to the account that's tracking circulation", async () => {
      const updateSend = AccountUpdate.createSigned(sender, tokenAContract.deriveTokenId());
      updateSend.balanceChange = Int64.fromUnsigned(sendAmount).neg();
      const updateReceive = AccountUpdate.create(tokenA, tokenAContract.deriveTokenId());
      updateReceive.balanceChange = Int64.fromUnsigned(sendAmount);
  ...
  describe("third party", () => {
    ...
    it("should deposit from the user to the token account of the third party", async () => {
      ...
      equal((await tokenAContract.getBalanceOf(thirdPartyA)).toBigInt(), depositAmount.toBigInt());
      equal(
        (await tokenAContract.getBalanceOf(sender)).toBigInt(),
    ...
    it("should send tokens from one contract to another", async () => {
      const initialBalance = (await tokenAContract.getBalanceOf(thirdPartyA)).toBigInt();
      const initialBalance2 = (await tokenAContract.getBalanceOf(thirdPartyB)).toBigInt();
      const initialCirculating = (await tokenAContract.getCirculating()).toBigInt();
  ...
  describe("Custom Admin Contract", () => {
    ...
    it("should mint with a custom admin contract", async () => {
      ...
      const initialBalance = (await tokenBContract.getBalanceOf(sender)).toBigInt();
    ...
    it("should send tokens without having the custom admin contract", async () => {
      const initialBalanceSender = (await tokenBContract.getBalanceOf(sender)).toBigInt();
      const initialBalanceReceiver = (await tokenBContract.getBalanceOf(receiver)).toBigInt();
      const initialCirculating = (await tokenBContract.getCirculating()).toBigInt();
      ...
      equal((await tokenBContract.getCirculating()).toBigInt(), initialCirculating);
    });
    ...
    it(
      "should not mint too many B tokens using the vanilla admin contract",
      {
        skip: !proofsEnabled,
      },
      async () => {
